...
import { Database, DbTransaction } from '@libs/db/types';
...
export class InternalRevenueService {
  ...
  private async _createTransaction(
    ...
    if (params.txDetails.for === 'tuyo')
      prevBalance = await tx.query.balances.findFirst({
        where: eq(balances.for, 'tuyo'),
      });
    ...
    if (!prevBalance)
      throw new Error(
        'Balance record not found for: ' + params.txDetails.for === 'tuyo'
          ? 'tuyo'
  ...
  private async _updateTransaction(
    ...
    if (params.balanceUpdate) {
      ...
      if (!prevBalance)
        throw new Error(
          'Balance record not found for: ' + prevTrx.for === 'tuyo'
            ? 'tuyo'
            : prevTrx.for === 'user'
  ...
  private async updateBalance(params: {
    ...
    if (params.balanceUpdate.crossTransfer) {
      ...
      let locked =
        ...
        (params.balanceUpdate.crossTransfer === 'locked-to-available'
          ? -params.amount
          : params.amount);
